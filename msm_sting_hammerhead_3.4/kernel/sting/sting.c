...
{
	int sn = syscall_get_nr(current, task_pt_regs(t));
	if (in_set(sn, create_set) || in_set(sn, use_set))
/* ARM XXX		bind_call(sn) || connect_call(sn)) */
		return 1;
	return 0;
}
...
void sting_syscall_begin(void)
{
	...
	char *pfname;
	int sn, sn_subtype;
	char *context = NULL; //gxp18
	u32 len; //gxp18

	pfname = kzalloc(256, GFP_ATOMIC);
	if (!pfname)
		return;

	sn = syscall_get_nr(current, task_pt_regs(current));
	sn_subtype = task_pt_regs(current)->ARM_r7; //gxp18 from r0 to r7

	//if (sn_subtype != 0) //gxp18
	//	printk("Sting sting.c sn_subtype= %i \n", sn_subtype); //gxp18



	//if (sn != 0) //gxp18
	//	printk("Sting sting.c SN= %i \n", sn); //gxp18
	...
	while (nd.last_type == LAST_BIND || !lc) {
		...
		if (lc && (nd.last_type == LAST_NORM || nd.last_type == LAST_BIND)) {
			...
			if (sting_adv_model->valid_adversary(adv_id)) {
				...
				// gxp 18 -  Remove Virtual Files
				if ((strncmp(fname, "/acct/", strlen("/acct/")) != 0) &&
				(strncmp(fname, "/dev/", strlen("/dev/")) != 0)) {

					char *adv_str = kasprintf(GFP_KERNEL, "{uid: {%d}, gid: {%d}}",
						uid_array[adv_id][0], uid_array[adv_id][1]);
	

					int sid = mac_get_sid(current->cred);
								
					int rc;
					char *scontext;
					u32 scontext_len;

					char* args = pft_get_process_argv_str(current);

					rc = security_sid_to_context(sid, &scontext, &scontext_len);
					
					STING_LOG("\nadversary: %s for uid: %d and Filename: %s process: %s pid: %d \nsid: %s syscall_no: %d args: %s\n",
						adv_str,
						current->cred->fsuid, fname, current->comm, 
						current->pid,
						scontext,
						sn, // print syscall instead
						args);
					kfree(scontext);
					break;
				}
